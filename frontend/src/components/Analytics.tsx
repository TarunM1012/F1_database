...
const Analytics: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-semibold text-gray-900 mb-4">External API Integration</h2>
        <p className="text-sm text-gray-600 mb-4">
          Fetch data from external APIs and store it in the database. Data is automatically saved and can be queried later.
        </p>
        
        <div className="mb-6">
          <h3 className="text-lg font-medium text-gray-900 mb-3">Fetch New Data</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button
              onClick={() => getExternalData('weather/Monaco')}
              disabled={loading}
            ...
            >
              {loading && externalDataType === 'weather/Monaco' ? 'Loading...' : 'Get Weather Data'}
            </button>
            <button
              onClick={() => getExternalData('news')}
              disabled={loading}
              className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {loading && externalDataType === 'news' ? 'Loading...' : 'Get F1 News'}
            </button>
            <button
              onClick={() => getExternalData('sentiment/Hamilton')}
              disabled={loading}
              className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {loading && externalDataType === 'sentiment/Hamilton' ? 'Loading...' : 'Sentiment Analysis'}
            </button>
          </div>
        </div>

        <div className="mb-6">
          <h3 className="text-lg font-medium text-gray-900 mb-3">Query Stored Data</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button
              onClick={() => getStoredData('weather', 'Monaco')}
              disabled={loading}
              className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {loading && externalDataType === 'stored_weather' ? 'Loading...' : 'Weather History'}
            </button>
            <button
              onClick={() => getStoredData('news')}
              disabled={loading}
              className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {loading && externalDataType === 'stored_news' ? 'Loading...' : 'Stored News'}
            </button>
            <button
              onClick={() => getStoredData('sentiment', 'Hamilton')}
              disabled={loading}
              className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed font-medium"
            >
              {loading && externalDataType === 'stored_sentiment' ? 'Loading...' : 'Sentiment History'}
            </button>
          </div>
        </div>
        ...
        {externalData && (
          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <h4 className="font-semibold text-gray-900 mb-3">
              {externalDataType.includes('stored') ? 'Stored Data Results' : 'API Response'}
            </h4>
            {externalData.error ? (
              <div className="alert alert-error">{externalData.error}</div>
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full">
                  ...
                  <tbody>
                    {externalData.slice(0, 10).map((item: any, idx: number) => (
                      <tr key={idx}>
                ...
                {externalData.length > 10 && (
                  <p className="text-sm text-gray-500 mt-2">Showing first 10 of {externalData.length} records</p>
                )}
              </div>
            ) : (
